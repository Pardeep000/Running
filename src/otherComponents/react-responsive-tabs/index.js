...
export default class Tabs extends Component {
  ...
  getTabs = () => {
    ...
    return items.reduce(
      (result, item, index) => {
        const { key = index, title, content, getContent, disabled, tabClassName, panelClassName ,color } = item;
  ...
  getTabProps = ({ title,color, key, selected, collapsed, tabIndex, disabled, className, onRemove, allowRemove,onChildrenClick }) => ({
    selected,
    allowRemove,
    children: title,
    key: tabPrefix + key,
  ...
  getClassNamesFor = (type, { selected, collapsed, tabIndex, disabled, color,className = '', isHidden }) => {
    const { tabClass, panelClass } = this.props;
    switch (type) {
      case 'tab':
        return cs(`RRT__tab RRT__tab_${color}`, className, tabClass, {
